...
func (a *App) registerServices() error {
	...
	do.Provide(a.injector, func(i do.Injector) (*lookup.Service, error) {
		...
		cfg := do.MustInvoke[*config.Config](i)
		return lookup.New(pg, redis, cfg.YandexDictionaryKey, cfg.YandexDictionaryURL), nil
	})

	// FCM Token services
	do.Provide(a.injector, func(i do.Injector) (*add_fcm_token.Service, error) {
		pg := do.MustInvoke[*postgres_sqlc.Postgres](i)
		return add_fcm_token.New(pg), nil
	})

	do.Provide(a.injector, func(i do.Injector) (*delete_fcm_token.Service, error) {
		pg := do.MustInvoke[*postgres_sqlc.Postgres](i)
		return delete_fcm_token.New(pg), nil
	})
